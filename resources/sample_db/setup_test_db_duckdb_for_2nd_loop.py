...
def initialize_sample_database(db_path: str, num_records: int = 1000):
    ...
    conn.execute(
        ...
        (
            ...
            WD_ACNT_NO      NVARCHAR(20),
            DPS_USR_NAME    NVARCHAR(50),
            DPS_BANK_CD     NVARCHAR(3),
            DPS_ACNT_NO     NVARCHAR(20),
            WD_MN           BIGINT,
            FEE             BIGINT
        )
        """
    ...
    hf_comments = [
        ...
        ("WD_ACNT_NO", "Withdrawal account number"),
        ("DPS_USR_NAME", "Depositor user name"),
        ("DPS_BANK_CD", "Deposit bank code"),
        ("DPS_ACNT_NO", "Deposit account number"),
    ...
    conn.execute(
        ...
        (
            ...
            USR_SRL_NO      NVARCHAR(10),
            CNTN_INFO       NVARCHAR(88),
            FIN_TECH_SVC_NO NVARCHAR(24),
            BLNC            BIGINT
        )
        """
    )
    ...
    ob_inqr_comments = [
        ...
        ("USR_SRL_NO", "User serial number(past user sequence number)"),
        ("CNTN_INFO", "CI - Connecting Information, identification value issued by identity verification agency"),
        ("FIN_TECH_SVC_NO", "Financial transfer service number"),
        ("BLNC", "Balance")
    ]
    ...
    ob_trns_comments = [
        ...
        ("WD_BANK_CD", "Withdrawal bank code"),
        ("FIN_TECH_SVC_NO", "Financial transfer service number"),
        ("USR_NAME", "Service user real name"),
        ("DPS_BANK_CD", "Deposit bank code"),
        ("DPS_ACNT_NO", "Deposit account number"),
        ("WD_MN", "Withdrawal amount")
    ]
    ...
    pi_comments = [
        ...
        ("TRNS_DTIME", "Transaction datetime(YYYYMMDDhhmmss)"),
        ("RCMS_BSWR_CL_CD", "Recipient inquiry classification code(01:Inquired, 02:Not inquired, 99:No information)"),
        ("PYR_NO", "Payer id number(past CMS number)"),
        ("USR_SRL_NO", "User serial number(past user sequence number)"),
        ("CNTN_INFO", "CNTN_INFO - Connecting Information, CI - Connecting Information, identification value issued by identity verification agency"),
        ("WD_BANK_CD", "Withdrawal bank code"),
        ("WD_ACNT_NO", "Withdrawal account number"),
        ("USR_NAME", "Service user real name"),
        ("DPS_BANK_CD", "Deposit bank code"),
    ...
    conn.execute("CREATE INDEX idx_hf_dps_usr_name ON HF_TRNS_TRAN(DPS_USR_NAME)")
    conn.execute("CREATE INDEX idx_hf_wd_acnt_no ON HF_TRNS_TRAN(WD_ACNT_NO)")

    # CD_TRNS_TRAN indexes
    conn.execute("CREATE INDEX idx_cd_trns_dtime ON CD_TRNS_TRAN(TRNS_DTIME)")
    conn.execute("CREATE INDEX idx_cd_usr_name ON CD_TRNS_TRAN(USR_NAME)")

    # OB_INQR_TRAN indexes
    conn.execute("CREATE INDEX idx_ob_inqr_dtime ON OB_INQR_TRAN(INQR_DTIME)")
    conn.execute("CREATE INDEX idx_ob_inqr_ci ON OB_INQR_TRAN(CNTN_INFO)")

    # OB_TRNS_TRAN indexes
    conn.execute("CREATE INDEX idx_ob_trns_dtime ON OB_TRNS_TRAN(TRNS_DTIME)")
    conn.execute("CREATE INDEX idx_ob_trns_ci ON OB_TRNS_TRAN(CNTN_INFO)")

    # PI_WD_LEDG indexes
    conn.execute("CREATE INDEX idx_pi_trns_dtime ON PI_WD_LEDG(TRNS_DTIME)")
    conn.execute("CREATE INDEX idx_pi_ci ON PI_WD_LEDG(CNTN_INFO)")

    # GR_JC_TRAN indexes
    conn.execute("CREATE INDEX idx_gr_jc_req_dt ON GR_JC_TRAN(REQ_DT)")
    conn.execute("CREATE INDEX idx_gr_jc_pyr_no ON GR_JC_TRAN(PYR_NO)")

    # GR_NJ_TRAN indexes
    conn.execute("CREATE INDEX idx_gr_nj_trns_dtime ON GR_NJ_TRAN(TRNS_DTIME)")
    conn.execute("CREATE INDEX idx_gr_nj_pyr_no ON GR_NJ_TRAN(PYR_NO)")

    # CMS_REQ_TRAN indexes
    conn.execute("CREATE INDEX idx_cms_req_wd_dt ON CMS_REQ_TRAN(WD_DT)")
    conn.execute("CREATE INDEX idx_cms_req_pyr_no ON CMS_REQ_TRAN(PYR_NO)")

    # CMS_RES_TRAN indexes
    conn.execute("CREATE INDEX idx_cms_res_wd_dt ON CMS_RES_TRAN(WD_DT)")
    conn.execute("CREATE INDEX idx_cms_res_pyr_no ON CMS_RES_TRAN(PYR_NO)")
    ...
    for i in range(num_records):
        record = (
            ...
            generate_account_num(),  # WD_ACNT_NO
            generate_korean_name(),  # USR_NAME
            random.choice(list(BANK_CODES.keys())),  # DPS_BANK_CD
            generate_account_num(),  # DPS_ACNT_NO
    ...
    result = conn.execute(
        """
        SELECT TRNS_DTIME, DPS_USR_NAME, WD_MN, TRNS_MED_CL_CD
